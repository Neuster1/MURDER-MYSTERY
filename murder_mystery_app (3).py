...
def screen_playing():
    ...
    with tabs[2]:
        ...
        # Track last interrogated suspect in session state (fixes nested-button bug)
        if st.button("👤 Interrogate", key="do_interrog"):
            ss.last_interrogated = chosen
            add_log(f"👤 Interrogated **{chosen}**", "info")
            st.rerun()

        # Show alibi and press-harder button OUTSIDE the interrogate button block
        target = ss.get("last_interrogated")
        if target and target in ss.suspect_data:
            d = ss.suspect_data[target]
            st.markdown(f"""
            <div class="det-card">
            <b>{target}</b> says:<br><br>
            <i>"{d['alibi']}"</i>
            </div>
            """, unsafe_allow_html=True)
            ...
            if d["secret_revealed"]:
                ...
                if st.button(f"🔥 Press {target} harder", key=f"press_{target}"):
                    d["secret_revealed"] = True
                    points(20, f"secret: {target}")
                    add_log(f"🗣 Uncovered **{target}'s** secret motive!", "secret")
                    st.rerun()
            else:
